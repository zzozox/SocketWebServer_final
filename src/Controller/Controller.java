...
//检查是否需要拦截请求,如果被拦截了，返回true；没被拦截，返回false
...
//登陆前拦截器,登录前仅允许访问登陆和注册页面,如果访问合法返回false，否则返回false并重定向回登陆页面
...
//登陆后拦截器
//登录后允许访问根目录下的资源，但是不允许路径中返回上级目录如果访问合法返回false，否则返回true并重定向主页
...
//检查是否可以返回304
//如果可以返回304，直接发回304状态报文并返回true；如果不能返回304，则返回false
